...
public class DLProxyImpl {
    ...
    public DLProxyImpl(Activity activity) { //activity DLProxyActivity
    ...
    public void onCreate(Intent intent) {
        ...
        intent.setExtrasClassLoader(DLConfigs.sPluginClassloader); //获取统一的classLoader

        mPackageName = intent.getStringExtra(DLConstants.EXTRA_PACKAGE);
        mClass = intent.getStringExtra(DLConstants.EXTRA_CLASS); // 要启动的class
        ...
        handleActivityInfo();
        launchTargetActivity(); // 去加载指定的class
    }
    ...
    protected void launchTargetActivity() {
        try {
            ...
            ((DLAttachable) mProxyActivity).attach(mPluginActivity, mPluginManager); // 关联上，代理类mProxyActivity，和实际执行的类mPluginActivity，
            ...
            mPluginActivity.onCreate(bundle); // 当Android系统调用这个类的activity， 就回调到apk 中要加载的类oncreate
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
