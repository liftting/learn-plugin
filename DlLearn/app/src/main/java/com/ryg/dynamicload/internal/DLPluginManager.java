...
public class DLPluginManager {
    ...
    /**
     * return value of {@link #startPluginActivity(android.app.Activity, com.ryg.dynamicload.internal.DLIntent)} start
     * success
     */
    public static final int START_RESULT_SUCCESS = 0;

    /**
     * return value of {@link #startPluginActivity(android.app.Activity, com.ryg.dynamicload.internal.DLIntent)} package
     * not found
     */
    public static final int START_RESULT_NO_PKG = 1;

    /**
     * return value of {@link #startPluginActivity(android.app.Activity, com.ryg.dynamicload.internal.DLIntent)} class
     * not found
     */
    ...
    private DLPluginPackage preparePluginEnv(PackageInfo packageInfo, String dexPath) {
        ...
        //为要加载的apk生成一些 loader， manager resource等
    ...
    /**
     * {@link #startPluginActivityForResult(android.app.Activity, com.ryg.dynamicload.internal.DLIntent, int)}
     */
    ...
    public int startPluginActivityForResult(Context context, DLIntent dlIntent, int requestCode) {
        ...
        // put extra data
        dlIntent.putExtra(DLConstants.EXTRA_CLASS, className); // 启动的类名
        dlIntent.putExtra(DLConstants.EXTRA_PACKAGE, packageName); //包名
        dlIntent.setClass(mContext, activityClass); // 看这里启动的类已经换了 ，是插件固定的两种类的类型， 即代理类，然后调用Android基本的处理类型，
        performStartActivityForResult(context, dlIntent, requestCode);
    ...
    public int startPluginService(final Context context, final DLIntent dlIntent) {
        ...
        fetchProxyServiceClass(dlIntent, new OnFetchProxyServiceClass() {
            ...
            public void onFetch(int result, Class<? extends Service> proxyServiceClass) {
                ...
                if (result == START_RESULT_SUCCESS) { // 当两个类都找到后，
    ...
    private void fetchProxyServiceClass(DLIntent dlIntent, OnFetchProxyServiceClass fetchProxyServiceClass) {
        ...
        Class<? extends Service> proxyServiceClass = getProxyServiceClass(clazz); //代理的类
    ...
    private String getPluginActivityFullPath(DLIntent dlIntent, DLPluginPackage pluginPackage) {
        String className = dlIntent.getPluginClass(); // 拿要启动的activity的名称，
...
//        isAssignableFrom针对class对象
        ...
        Class<? extends Activity> activityClass = null; // 先看传递过来的class，是否继承了 特定的一些类，
        if (DLBasePluginActivity.class.isAssignableFrom(clazz)) {
            activityClass = DLProxyActivity.class; // 启动的是代理的activity
